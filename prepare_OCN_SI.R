...
# prepare OCN ####
# new OCN with Qo =2.5 m3/s
# Let's assume rectangular section at the outlet with w=10 m
# Q = Ks * S^0.5 * RH^2/3 -> with Q=10, w=10 it is KS*S^0.5=1.13
# then with Q'=2.5 m3/s, it is h'=0.418 m, v'=0.599 m/s
# here we don't calculate RN velocities

# note: recreating OCN_eDNA takes around 1 day
if (!file.exists("OCN_eDNA_SI.rda")){
  load(file="OCN_eDNA.rda")
  # recalculate hydraulic properties with Q0=2.5 m3s-1
  OCN <- rivergeometry_OCN(OCN, widthMax = 10, velocityMax = 0.599, depthMax = 0.418)
  ...
  save(OCN,file="OCN_eDNA_SI.rda")
} else {
  load(file="OCN_eDNA_SI.rda")
}
